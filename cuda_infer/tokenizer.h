...
int  bpe_decode_token(const bpe_tokenizer *tok, uint32_t token_id, char *buf, int buf_size);
...
int bpe_decode_token(const bpe_tokenizer *tok, uint32_t token_id, char *buf, int buf_size) {
    for (uint32_t i = 0; i < tok->vocab_size; i++) {
        if (tok->vocab[i].id == token_id) {
            const char *s = tok->vocab[i].str;
            uint16_t slen = tok->vocab[i].len;
            int pos = 0;
            for (int j = 0; j < slen && pos < buf_size - 1; ) {
                uint8_t c = (uint8_t)s[j];
                uint32_t cp;
                int clen;
                if (c < 0x80) {
                    cp = c; clen = 1;
                } else if ((c & 0xE0) == 0xC0 && j + 1 < slen) {
                    cp = ((uint32_t)(c & 0x1F) << 6) | (s[j+1] & 0x3F);
                    clen = 2;
                } else if ((c & 0xF0) == 0xE0 && j + 2 < slen) {
                    cp = ((uint32_t)(c & 0x0F) << 12) | ((uint32_t)(s[j+1] & 0x3F) << 6) | (s[j+2] & 0x3F);
                    clen = 3;
                } else if ((c & 0xF8) == 0xF0 && j + 3 < slen) {
                    cp = ((uint32_t)(c & 0x07) << 18) | ((uint32_t)(s[j+1] & 0x3F) << 12) | ((uint32_t)(s[j+2] & 0x3F) << 6) | (s[j+3] & 0x3F);
                    clen = 4;
                } else {
                    j++;
                    continue;
